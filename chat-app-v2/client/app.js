...
const initAuth = () => {
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', async (e) => {
            ...
            if (!pwdRegex.test(password)) {
                showError('Password must contain at least 8 characters, one uppercase, one lowercase, and one number.');
                return;
            }
    ...
    const authInputs = document.querySelectorAll('#auth-panel input');
    authInputs.forEach(input => {
        ...
        input.addEventListener('paste', (e) => {
            e.preventDefault();
            const text = (e.originalEvent || e).clipboardData.getData('text/plain').replace(/\\s/g, '');
